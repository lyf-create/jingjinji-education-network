...
const CollaborationNetworkSearchFix=(function(){
  ...
  function init(root,data,app){
    ...
    function executeFixedSearch(){
      ...
      if(!keyword)return;
      /* Full-name searches always start from the complete event library. */
      const reset=(selector,value,eventName)=>{const control=root.querySelector(selector);if(!control||control.value===value)return;control.value=value;if(typeof control[eventName]==='function')control[eventName]({target:control});};
      reset('.collab-network__select','ALL','onchange');
      reset('[data-domain]','全部','onchange');
      reset('[data-region]','全部','onchange');
      reset('[data-type]','全部','onchange');
      reset('[data-min]','1','oninput');
      const exact=(data.nodes||[]).find(n=>String(n.name||'').trim()===keyword);
      const result=exact?{target:exact,matches:[exact]}:findTarget(keyword);
      if(result.target){
